...
/**
 * EasyWechat使用redis进行缓存
 *
 * Class EasyWeChatCache
 * @package App\Bean\Singleton
 * Created by lujianjin
 * DataTime: 2020/10/8 11:09
 *
 * @Bean(name="easyWeChatCache")
 */
class EasyWeChatCache implements CacheInterface
{
    ...
    public function getMultiple($keys, $default = null)
    {
        ...
        $this->redis->pipeline(function (Redis $redis) use ($keys, &$resultArr, $default) {
            foreach ($keys as $key) {
                $resultArr[] = $redis->get($key) ?: $default;
    ...
    {
        $this->redis->pipeline(function (Redis $redis) use ($values, $ttl) {
            foreach ($values as $key => $value) {
                $redis->set($key, $value, $ttl) ?: false;
